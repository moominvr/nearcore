...
impl Handler<NetworkClientMessages> for ClientActor {
    ...
    fn handle(&mut self, msg: NetworkClientMessages, _ctx: &mut Context<Self>) -> Self::Result {
        match msg {
            ...
            NetworkClientMessages::AdvProduceBlocks(num_blocks, is_chain) => {
                info!(target: "adversary", "Producing {} blocks", num_blocks);
                self.client.adv_produce_blocks = true;
                for i in 0..num_blocks {
                    let mut height =
                        self.client.chain.mut_store().get_latest_known().unwrap().height;
                    if is_chain {
                        height += 1;
                    }
                    info!(target: "adversary", "Producing {} block out of {}, height = {}", i, num_blocks, height);
                    let elapsed = self.client.config.max_block_production_delay;
                    let block = self
                        .client
                        .produce_block(height, elapsed)
                        .expect("block should be produced")
                        .expect("block should exist after produced");
